...
function Snippet_TopBarPlayerSlot_Update(panel) {
	...
	if (players_abandoned.indexOf(playerId) !== -1) {
		...
		panel.FindChildTraverse('PlayerColor').style.backgroundColor = playerInfo.player_team_id && teamColors[playerInfo.player_team_id] || "#ffff00";
...
function Snippet_DotaTeamBar(team) {
	if (TeamPanels[team] == null) {
		...
		panel.team = team;
		panel.FindChildTraverse('TopBarScore').style.textShadow = '0 0 10px ' + teamColors[team];
...
function Update() {
	...
	$.Each(Game.GetAllPlayerIDs(), function(playerId) {
